...
traj_MOT_dir <- here("data","exp1","trajectories")
traj_plot_dir <- here("plots","exp1","trajectories")
...
sett_generate <-
  new_settings(xlim = c(-500, 500), ylim = c(-362, 362), min_distance = 100,
               arena_shape = "square")
...
random_coords_in_square <- function(n, xmin, xmax, ymin, ymax) {
  quadrant_order <- seq(1,4)
  quadrant_order = sample(quadrant_order)
  t <- tibble()
  ...
  for (target_vs_distractors in 1:2) {
    for (i in 1:4) {
      ...
      if (q == 1) {
        ...
        x = stats::runif(1, min = 37, max = 590)
        y = stats::runif(1, min = 37, max = 462)
      }
      else if (q == 3) {
        x = stats::runif(1, min = -590, max = -37)
        y = stats::runif(1, min = -462, max = -37)
      }
      else if (q == 4) {
        x = stats::runif(1, min = 37, max = 590)
        y = stats::runif(1, min = -462, max = -37)
...
make_random_trajectoryJJC <- function(start, times, settings, step_function, ...) {
  ...
  trajectoryLength <- ((length(times) - 1) * 8) + 1

  duration_of_final_interval_when_bounces_are_prohibited <- 0.2 #Don't allow any bounces off walls, corners, or other balls in the last interval of the trial
...
# 
# psychopy_dir    <- here("psychopy","exp1")
# psychopy_trajectories <- file.path(psychopy_dir, "trajectories")
# 
# if(!dir.exists(psychopy_trajectories)) { dir.create(psychopy_trajectories, recursive = T) }
# 
# # copy trajectories
# trajectories_files_from <- list.files(traj_MOT_dir,full.names = T)
# trajectories_files_to   <-
#   trajectories_files_from %>%
#   basename() %>%
#   file.path(psychopy_trajectories,.)
# file.copy(trajectories_files_from, trajectories_files_to,overwrite = T)
