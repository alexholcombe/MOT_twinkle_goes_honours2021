...
win = visual.Window(
    ...
    monitor='philipa', color=[0.004,0.004,0.004], colorSpace='rgb',
...
#prot_file = 'protocols/P001_noise.csv'
...
noise = visual.NoiseStim(
    ...
    noiseType='Uniform', noiseElementSize=[0.00625], 
    ...
    noiseClip=3.0, imageComponent='Phase', interpolate=False, depth=0.0, units = 'norm')
noise.buildNoise()
...
for thisPractice_trial in practice_trials:
    ...
    win.blendMode = 'add'
    # image.setImage(noise_backg_pth) JJC
    # keep track of which components have finished
    ...
    #Set up cage postcue
    p1 = visual.Rect(
            win=win, name='p1',units='norm', size=(1, 1),
            ori=0, pos=(0.5, 0.5),
    ...
    mouseHighlight = visual.Circle(
            win=win, name='mouseHighlight',units='pix', radius=12,
            pos=(0, 0),
    ...
    trialComponents = [o1, mouse, noise, h_line, v_line,p1]
    ...
    while continueRoutine:
        ...
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        #noise_backg.setAutoDraw(True) JJC
        #noise_backg.setAutoDraw(False)
        noise.draw( )
        
        current_trial_static_noise = noise
        ...
        if v_line.status == STARTED and h_line.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > v_line.tStartRefresh + 100.0-frameTolerance:
                # keep track of stop time/frame for later
                v_line.tStop = t  # not accounting for scr refresh
                v_line.frameNStop = frameN  # exact frame index
                win.timeOnFlip(v_line, 'tStopRefresh')  # time at next scr refresh
                v_line.setAutoDraw(False)
                h_line.tStop = t  # not accounting for scr refresh
                h_line.frameNStop = frameN  # exact frame index
                win.timeOnFlip(h_line, 'tStopRefresh')  # time at next scr refresh
                h_line.setAutoDraw(False)
        o1.setAutoDraw(False)
...
#                    # is it time to stop? (based on global clock, using actual start)
#                    if tThisFlipGlobal > noise.tStartRefresh + 100-frameTolerance:
#                        # keep track of stop time/frame for later
...
#                        noise.setAutoDraw(False)
            ...
            if (nClicks < 1):  # repeat, until participant clicked on 4 objects
                ...
                if mouse.isPressedIn(p1, buttons=[0]):
                    x, y = mouse.getPos()
                    nClicks = nClicks + 1
...
#                    P.objects[i2].draw() 
            ...
            else: # a click was made, ending routine
                win.mouseVisible = False
                continueRoutine = False
                for i2 in range(n_objects):
        ...
        # updates p1 log of information when it is first drawn, and starts drawing it
